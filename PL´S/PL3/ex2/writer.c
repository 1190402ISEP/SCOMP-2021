...
int main() {
    ...
    Product *memoriaPartilhada = (Product *) mmap(NULL, TAMANHO_ESTRUTURA, PROT_READ | PROT_WRITE, MAP_SHARED,
                                                  fileDescriptor, 0);

    //Interação com o Utilizador começa aqui:

    printf("Insira o código do produto:\n");
    scanf("%s", memoriaPartilhada->code);

    printf("Insira a descrição do Produto:\n");
    scanf("%s", memoriaPartilhada->description);

    printf("Insira o preço do Produto:\n");
    scanf("%f", &memoriaPartilhada->price);
